...
- **Trigger name**: `trg_calendar_{descriptive_name}`
- **Function name**: `trg_calendar_{descriptive_name}_fn`

Where `calendar` is your plugin's moduleId (pluginId with hyphens replaced by underscores).
...
CREATE OR REPLACE FUNCTION "trg_calendar_audit_log_fn"()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER "trg_calendar_audit_log"
AFTER INSERT OR UPDATE ON "module_calendar_items"
FOR EACH ROW
EXECUTE FUNCTION "trg_calendar_audit_log_fn"();
```

> **Important**: The system automatically **DISABLE**s all `trg_calendar_*` triggers when the plugin
